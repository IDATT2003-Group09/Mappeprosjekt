...
class GameProgressTest {
    ...
    private static final BigDecimal BASE_REQUIREMENT = new BigDecimal("0.10");
    private static final BigDecimal STARTING_MONEY = new BigDecimal("1000.00");
    ...
    class ConstructorTests {
        ...
        void twoArgConstructor_setsInitialState() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);

            assertEquals(0, gp.getCurrentWeek());
            assertEquals(1, gp.getCheckpointLevel());
            assertEquals(1, gp.getLastCalculatedLevel());
            assertEquals(BASE_REQUIREMENT, gp.getBaseRequirement());
        }
        ...
        void threeArgConstructor_weekZero_matchesTwoArg() {
            GameProgress twoArg   = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            GameProgress threeArg = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY, 0);

            assertEquals(twoArg.getCurrentWeek(),     threeArg.getCurrentWeek());
            assertEquals(twoArg.getCheckpointLevel(), threeArg.getCheckpointLevel());
            assertEquals(0, twoArg.getCurrentTarget().compareTo(threeArg.getCurrentTarget()));
        }
        ...
        void threeArgConstructor_midQuarter_correctCheckpoint() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY, 7);
        ...
        void threeArgConstructor_atQuarterBoundary_advancesLevel() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY, 13);
        ...
        void threeArgConstructor_negativeWeek_clampedToZero() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY, -5);
        ...
        void constructor_currentTarget_calculatedCorrectly() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            BigDecimal expected = STARTING_MONEY.multiply(
                BigDecimal.ONE.add(BASE_REQUIREMENT).pow(1)
            );

            assertEquals(0, expected.compareTo(gp.getCurrentTarget()));
        }
    }
    ...
    class WeekProgressionTests {
        ...
        void isQuarterComplete_falseBeforeCheckpoint() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            for (int i = 0; i < 12; i++) gp.nextWeek();
        ...
        void isQuarterComplete_trueAtCheckpoint() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            for (int i = 0; i < 13; i++) gp.nextWeek();
        ...
        void isQuarterComplete_truePastCheckpoint() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            for (int i = 0; i < 20; i++) gp.nextWeek();
            assertTrue(gp.isQuarterComplete());
        }
    }
    ...
    class RequirementTests {
        ...
        void meetsRequirement_belowTarget_returnsFalse() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            assertFalse(gp.meetsRequirement(new BigDecimal("500.00")));
        }
        ...
        void meetsRequirement_exactTarget_returnsTrue() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            assertTrue(gp.meetsRequirement(gp.getCurrentTarget()));
        }
        ...
        void meetsRequirement_aboveTarget_returnsTrue() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            assertTrue(gp.meetsRequirement(gp.getCurrentTarget().add(BigDecimal.ONE)));
        }
    }
    ...
    class AdvanceCheckpointTests {
        ...
        void advanceCheckpoint_advancesCheckpointWeek() {
            ...
            gp.advanceCheckpoint();
            assertEquals(weeksBefore + 13, gp.getWeeksUntilDeadline());
        }

        @Test
        @DisplayName("Doubles the base requirement")
        void advanceCheckpoint_doublesBaseRequirement() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            BigDecimal before = gp.getBaseRequirement();
            gp.advanceCheckpoint();
            assertEquals(0, before.multiply(BigDecimal.valueOf(2)).compareTo(gp.getBaseRequirement()));
        }
        ...
        void advanceCheckpoint_increasesCurrentTarget() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            BigDecimal targetBefore = gp.getCurrentTarget();
            gp.advanceCheckpoint();
            assertTrue(gp.getCurrentTarget().compareTo(targetBefore) > 0);
        }

        @Test
        @DisplayName("Multiple advances keep doubling the requirement")
        void advanceCheckpoint_multipleAdvances_keepDoubling() {
            GameProgress gp = new GameProgress(BASE_REQUIREMENT, STARTING_MONEY);
            gp.advanceCheckpoint();
            gp.advanceCheckpoint();
            BigDecimal expected = BASE_REQUIREMENT
                .multiply(BigDecimal.valueOf(2))
                .multiply(BigDecimal.valueOf(2));
            assertEquals(0, expected.compareTo(gp.getBaseRequirement()));
        }
    }
    ...
    class LastCalculatedLevelTests {
        ...
        void setAndGetLastCalculatedLevel() {
            ...
            assertEquals(42, gp.getLastCalculatedLevel());
        }
    }
}